...
$query ="SELECT * FROM tblConnote left join tblCity on ConnoteDest=CityId
			...
			and ConnoteCountDeli=0 LIMIT 10";
...
while ($data = mysql_fetch_assoc($sql)) {
	...
	$length='0'; //(is_numeric($data['VOLUME_L1']) && $data['VOLUME_L1'] < 0)?0:$data['VOLUME_L1'];
	$width='0'; //(is_numeric($data['VOLUME_W1']) && $data['VOLUME_W1'] < 0)?0:$data['VOLUME_W1'];
	$height='0'; //(is_numeric($data['VOLUME_H1']) && $data['VOLUME_H1'] < 0)?0:$data['VOLUME_H1'];
	$volume='1'; //(is_numeric($data['VOLUME_KG1']) && $data['VOLUME_KG1'] < 0)?0:$data['VOLUME_KG1'];
	$telpcust=trim($data['ConnoteCustAddr3']);
	if ($telpcust==''){
		$telpcust='999';
	}
	$telprecv=trim($data['ConnoteRecvReff']);
	if ($telprecv==''){
		$telprecv='999';
	}
	...
	$telpon1_pengirim=preg_replace('/[^0-9]/', '', $telpcust);

	$telpon2_pengirim=preg_replace('/[^0-9]/', '', $telpcust);
	...
	$telpon1_penerima=preg_replace('/[^0-9]/', '', $telprecv);
	$telpon2_penerima=preg_replace('/[^0-9]/', '', $telprecv);
	...
	curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://demo.pandulogistics.com/pandu/restapi/basic/raw_data/shipment_text_gen_pliss',
	...
	if ($status === 'POLICYS') { /* Handle error */ 
		...
		if ($status1 === 'success'){
			...
			$sql2  = mysql_query($query,$koneksi);		
			...
			if ((strpos($error1, 'Exists ') !== false) || (strpos($error1, 'duplicate ') !== false)) {
				$query = "update tblConnote set ConnoteCountDeli = '1' where ConnoteNo = '".$rec_id."'";
				$sql2  = mysql_query($query,$koneksi);		
			} else {
