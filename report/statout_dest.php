...
<html>
   ...
   function myparser(s){
    ...
    <form action="fmenu.php?hal=statoutdest" method="post">
    <table width="100%">
    <tr>
    <td style="font-size:16px"><h1>Statistik Outbound Shipment per Tujuan</h1></td>
    </tr>
    <tr>
    <td><hr></td>
    </tr>
            ...
            <table id="datatables" class="display">
                <thead>
                    <tr>
                        ...
                        <th align="left">Nama Kota/Tujuan</th>
                ...
                <tbody>
                    ...
                    <?php
						...
						$sql = mysql_query("SELECT ConnoteDest,CityCode,CityName,sum(ConnoteWeight) as berat, count(ConnoteNo) as banyak, sum(ConnoteBillAmount-ConnoteBillDisc) as nilai FROM tblconnote left join tblcity on ConnoteDest=CityId WHERE ConnoteDate BETWEEN '$t1' AND '$t2' AND ConnoteOrig=$location AND ConnoteValid>=0 GROUP BY ConnoteDest");
  						}
  					else {
						include('config/koneksi.php');
						$sql = mysql_query("SELECT ConnoteDest,CityCode,CityName,sum(ConnoteWeight) as berat, sum(ConnoteNo) as banyak, sum(ConnoteBillAmount-ConnoteBillDisc) as nilai FROM tblconnote left join tblcity on ConnoteDest=CityId WHERE ConnoteNo='NA#' AND ConnoteOrig=$location AND ConnoteValid=1 GROUP BY ConnoteDest");
  						}
