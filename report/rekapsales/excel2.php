...
if ($date1 >= '2022-09-01'){
$rs=$koneksi->query("SELECT CustomerKelola,sum(ConnoteBillAmount) AS NILAI, sum(ConnoteBillDisc) AS DISC, sum(ConnoteBillAmount-ConnoteBillDisc) AS NET, sum(ConnoteBillPack+ConnoteBillInsurance+ConnoteBillOther) AS OTH, sum(ConnoteWeight) as BERAT, count(ConnoteNo) as SHIP
          ...
          GROUP BY CustomerKelola ORDER BY NILAI DESC");
    ...
    <div id="printable">
        <table border='0' style="font-size:0.8em;border-collapse: collapse;" cellpadding="0">
            ...
            ?>
                <tr>
                    <td><?php echo $no ?></td>
                    <td><?php echo $data_connote->CustomerKelola ?></td>
